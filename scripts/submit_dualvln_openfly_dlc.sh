...
EVAL_NAME="${EVAL_NAME:-dualvln_openfly_${EVAL_ENV}_$(date +%Y%m%d_%H%M%S)}"
JOB_NAME="${JOB_NAME:-${EVAL_ENV//_/-}-dualvln-$(date +%Y%m%d-%H%M%S)}"
...
worker_command="env MODEL_GPU=${MODEL_GPU} SIM_GPU=${SIM_GPU} MAX_SAMPLES=${MAX_SAMPLES} MAX_STEPS=${MAX_STEPS} STOP_THRESHOLD=${STOP_THRESHOLD} EVAL_ENV=${EVAL_ENV} CONFIG=${CONFIG} SOURCE_EVAL_CONFIG=${SOURCE_EVAL_CONFIG} RESULT_ROOT=${RESULT_ROOT} bash ${AEROLOOP_ROOT}/scripts/dualvln_openfly_dlc_worker.sh"
