...
EVAL_NAME="${EVAL_NAME:-pi0_openfly_${EVAL_ENV}_$(date +%Y%m%d_%H%M%S)}"
JOB_NAME="${JOB_NAME:-${EVAL_ENV//_/-}-pi0-$(date +%Y%m%d-%H%M%S)}"
...
worker_command="env MODEL_GPU=${MODEL_GPU} SIM_GPU=${SIM_GPU} MAX_SAMPLES=${MAX_SAMPLES} PI0_HISTORY_STEPS=${PI0_HISTORY_STEPS} PI0_CHECKPOINT=${PI0_CHECKPOINT} EVAL_ENV=${EVAL_ENV} CONFIG=${CONFIG} SOURCE_EVAL_CONFIG=${SOURCE_EVAL_CONFIG} RESULT_ROOT=${RESULT_ROOT} bash ${AEROLOOP_ROOT}/scripts/pi0_openfly_dlc_worker.sh"
