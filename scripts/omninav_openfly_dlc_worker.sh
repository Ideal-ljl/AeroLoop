...
OMNINAV_ROOT="${OMNINAV_ROOT:-/mnt/petrelfs/youzhongrui/v2/Ab_ex/OmniNav}"
...
tmp.replace(final)
print(f"[omninav-worker] compacted {len(episodes)} unique episodes to {final}", flush=True)
PY
}
...
print(
    f"[omninav-worker] requested={len(selected)} completed={len(completed)} remaining={len(remaining)}",
    flush=True,
)
PY
...
echo "[omninav-worker] config=${CONFIG} env=${EVAL_ENV} samples=${MAX_SAMPLES} result=${RESULT_ROOT}"
nvidia-smi || true
...
PYTHONNOUSERSITE=1 \
"${SERVER_PYTHON}" -m aeroloop.server_cli omninav \
  --repo-root "${OMNINAV_ROOT}" \
  --ckpt-dir "${OMNINAV_ROOT}/checkpoint-19805" \
  --device cuda \
  --dtype bfloat16 \
  --attn-impl sdpa \
  --history-size 5 \
  --current-long-edge 640 \
  --host 127.0.0.1 --port "${SERVER_PORT}" \
  >"${RESULT_ROOT}/omninav_server.log" 2>&1 &
server_pid=$!
...
while time.time() < deadline:
    try:
        ...
        if payload.get("status") == "ok":
            print(f"[omninav-worker] health: {payload}", flush=True)
            break
    except Exception as exc:
        print(f"[omninav-worker] waiting for model: {exc}", flush=True)
    time.sleep(10)
else:
    raise TimeoutError("OmniNav did not become healthy in 900 seconds")
PY
