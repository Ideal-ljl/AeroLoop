...
MANIFEST="${MANIFEST:-${AEROLOOP_ROOT}/eval_results/dlc/omninav_openfly_matrix_${EVAL_TAG}.tsv}"
ENVIRONMENTS=(
  env_airsim_16
  env_airsim_18
  env_airsim_26
...
for eval_env in "${ENVIRONMENTS[@]}"; do
  eval_name="omninav_openfly_${eval_env}_${EVAL_TAG}"
  job_name="${eval_env//_/-}-omninav-${EVAL_TAG//_/-}"
  ...
  output="$(
    ...
    RESULT_ROOT="${result_root}" \
      "${AEROLOOP_ROOT}/scripts/submit_omninav_openfly_dlc.sh"
  )"
  printf '%s\n' "${output}"
