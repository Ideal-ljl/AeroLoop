...
DUALVLN_ROOT="${DUALVLN_ROOT:-${AB_EX_ROOT}/DualVLN}"
DUALVLN_STAGE1="${DUALVLN_STAGE1:-${AB_EX_ROOT}/ckpt/DualVLN/stage1_processor}"
DUALVLN_STAGE2="${DUALVLN_STAGE2:-${AB_EX_ROOT}/checkpoints/DualVLN/stage2_dualvln_bbox_delta_b12_16gpu/checkpoint-35000}"
...
CONFIG="${CONFIG:-${AEROLOOP_ROOT}/configs/jobs/dualvln_openfly.yaml}"
...
STOP_THRESHOLD="${STOP_THRESHOLD:-0.7}"
SERVER_PORT="${SERVER_PORT:-18103}"
MODEL_GPU="${MODEL_GPU:-0}"
SIM_GPU="${SIM_GPU:-0}"
EVAL_ENV="${EVAL_ENV:-env_airsim_18}"
PREDICT_STEPS="${PREDICT_STEPS:-4}"
DENOISE_STEPS="${DENOISE_STEPS:-10}"
SAMPLE_TRAJECTORIES="${SAMPLE_TRAJECTORIES:-32}"
...
CKPT_BUNDLE="${RESULT_ROOT}/dualvln_ckpt"
...
export MAX_STEPS STOP_THRESHOLD

mkdir -p "${CKPT_BUNDLE}"
ln -sfn "${DUALVLN_STAGE1}" "${CKPT_BUNDLE}/stage1"
ln -sfn "${DUALVLN_STAGE2}" "${CKPT_BUNDLE}/stage2"
...
tmp.replace(final)
print(f"[dualvln-worker] compacted {len(episodes)} unique episodes to {final}", flush=True)
PY
}
...
if len(selected) != max_samples:
    raise ValueError(
        f"{eval_env} selected {len(selected)} samples from {source_path}, expected {max_samples}"
    )
...
config["rollout"]["max_steps"] = int(os.environ["MAX_STEPS"])
config["rollout"]["stop_threshold"] = float(os.environ["STOP_THRESHOLD"])
common_camera = {
    "name": "front",
    "width": 896,
    "height": 896,
...
print(
    f"[dualvln-worker] requested={len(selected)} completed={len(completed)} remaining={len(remaining)}",
    flush=True,
)
PY
...
echo "[dualvln-worker] config=${CONFIG} env=${EVAL_ENV} samples=${MAX_SAMPLES} result=${RESULT_ROOT}"
nvidia-smi || true
...
PYTHONNOUSERSITE=1 \
"${SERVER_PYTHON}" -m aeroloop.server_cli dualvln \
  --repo-root "${DUALVLN_ROOT}" \
  --ckpt-dir "${CKPT_BUNDLE}" \
  --device cuda \
  --dtype bfloat16 \
  --predict-steps "${PREDICT_STEPS}" \
  --inference-steps "${DENOISE_STEPS}" \
  --sample-trajectories "${SAMPLE_TRAJECTORIES}" \
  --host 127.0.0.1 --port "${SERVER_PORT}" \
  >"${RESULT_ROOT}/dualvln_server.log" 2>&1 &
server_pid=$!
...
while time.time() < deadline:
    try:
        ...
        if payload.get("status") == "ok":
            print(f"[dualvln-worker] health: {payload}", flush=True)
            break
    except Exception as exc:
        print(f"[dualvln-worker] waiting for model: {exc}", flush=True)
    time.sleep(10)
else:
    raise TimeoutError("DualVLN did not become healthy in 900 seconds")
PY
