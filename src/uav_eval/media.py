...
class FFmpegVideoWriter:
    ...
    def _prepare(frame):
        try:
            ...
            raise RuntimeError("video output requires uav-eval[media]") from exc
...
def render_overlay(image, episode: EpisodeSpec, observation: Observation, record: StepRecord | None):
    try:
        ...
        raise RuntimeError("visual overlays require uav-eval[media]") from exc
    ...
    if record is not None:
        ...
        lines.extend(
            [
                ...
                f"distance={record.distances.get('legacy_min', float('nan')):.2f} collision={record.collision}",
...
class MediaObserver(RolloutObserver):
    ...
    def on_episode_start(self, episode: EpisodeSpec, observation: Observation) -> None:
        ...
        if self.config.save_video:
            video_path = Path(self.config.video_dir) / f"{_safe_name(episode.env_name)}__{_safe_name(episode.episode_id)}.mp4"
    ...
    def on_step(self, episode: EpisodeSpec, observation: Observation, record: StepRecord) -> bool:
        ...
        if record.collision and self.config.save_collision_frame and not self._collision_saved and frame is not None:
            ...
            path = Path(self.config.collision_dir) / f"{_safe_name(episode.env_name)}__{_safe_name(episode.episode_id)}.png"
