...
def heading_delta_from_translation(dx: float, dy: float, epsilon: float = 1e-8) -> float:
    """Derive a body-frame heading delta from an XY translation."""
...
class PredictRequest:
    ...
    image_history_base64: Mapping[str, tuple[str, ...]] = field(default_factory=dict)
    ...
    def from_mapping(cls, row: Mapping[str, Any]) -> "PredictRequest":
        state = tuple(map(float, row.get("state", (0, 0, 0, 0))))
        pose = tuple(map(float, row.get("pose", (0, 0, 0, 0))))
        ...
        raw_image_history = row.get("image_history_base64") or {}
        if not isinstance(raw_image_history, Mapping):
            raise ValueError("image_history_base64 must be an object keyed by view name")
        image_history = {}
        for name, values in raw_image_history.items():
            if not isinstance(values, (list, tuple)) or not all(isinstance(value, str) for value in values):
                raise ValueError(f"image history for view {name!r} must be a list of base64 strings")
            image_history[str(name)] = tuple(values)
        ...
        policy_context = row.get("policy_context") or {}
        ...
        return cls(
            ...
            image_history_base64=image_history,
    ...
    def decode_rgb_history(self, view: str | None = None):
        """Decode an oldest-to-current temporal image sequence for one view."""

        selected_view = view or self.primary_view
        encoded_frames = self.image_history_base64.get(selected_view, ())
        if not encoded_frames:
            return [self.decode_rgb(selected_view)]
        try:
            import numpy as np
            from PIL import Image
        except ImportError as exc:  # pragma: no cover
