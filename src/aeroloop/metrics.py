...
class MetricConfig:
    ...
    distance_mode: str = "endpoint_3d"
    require_stop_for_success: bool = False
    reference_path_offset: float = 0.0

    def __post_init__(self) -> None:
        valid = {"endpoint_2d", "endpoint_3d"}
        ...
        if self.reference_path_offset < 0:
            raise ValueError("reference_path_offset cannot be negative")

    @classmethod
    def from_mapping(cls, row: Mapping[str, Any]) -> "MetricConfig":
        values = dict(row)
        profile = str(values.pop("profile", "standard"))
        presets = {
            "standard": {},
            "openfly": {"success_distance": 25.0, "distance_mode": "endpoint_3d"},
            "traveluav": {
                "success_distance": 20.0,
                "distance_mode": "endpoint_3d",
                "require_stop_for_success": True,
                "reference_path_offset": 20.0,
            },
        }
        if profile not in presets:
            raise ValueError(f"unknown metric profile {profile!r}; choose from {sorted(presets)}")
        return cls(**{**presets[profile], **values})
...
class EpisodeMetrics:
    ...
    def finalize(self, termination: TerminationReason, final_pose: Pose, step_count: int) -> dict[str, Any]:
        ...
        ref = max(float(self.episode.reference_path_length) - self.config.reference_path_offset, 0.0)
        ...
        return {
            ...
            "ne": float(self.last_distances[self.config.distance_mode]),
